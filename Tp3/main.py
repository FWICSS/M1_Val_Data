...
with open('PRSA.csv', newline='') as csvfile:
    ...
    for row in reader :
                test = False
                for val in range(len(valcbwd)):
                    if(valcbwd[val] == row['cbwd']):
                        test = True
                if(test == False):
                    valcbwd.append(row['cbwd'])
                if(row['pm2.5'] == 'NA'):
                    nbVal+=1
                else :
                    if (nbVal > 0):
    ...
    for inc in range(len(valI)) :
        if (NbValManquante[inc]>15):
            ...
            for j in range(NbValManquante[inc]):
                b = df.loc[df["No"] == DebTrou[inc] - 1]
                df.loc[df["No"] == DebTrou[inc] + j, "pm2.5"] = float(b["pm2.5"]+valI[inc]*(j+1))
    df.to_csv("AIGLE_tp3_1.csv", index=False)
    print("Save on file AIGLE_tp2_1.csv")
    nbNA = 0
    with open('AIGLE_tp3_1.csv', newline='') as tpfile:
